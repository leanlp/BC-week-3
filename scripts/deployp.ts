...
const TOKENCONT = "0xCeB28a390E1F04aCcC6896d92cD5DcE9ec618964"
...
async function main()  {
    ...
    const BLOCKNUMBER = provider.getBlockNumber.length
    console.log(`${BLOCKNUMBER}`)

    const ballotContractFactory = new TokenizedBallot__factory(signer);
    const ballotContract = await ballotContractFactory.attach(TOKENCONT);
    // const delegateTx = await ballotContract.functions; //note ".address not write"
// await delegateTx.wait();

    // .deploy(
    // convertStringArrayToBytes32(PROPOSALS), TOKENCONT, BLOCKNUMBER);
